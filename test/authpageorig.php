...
if (isset($_REQUEST['logout'])) {
  phpCAS::logoutWithRedirectService('http://newsdevl.uthsc.edu');
}
?>
<html>
  <head>
  ...
  <body>
		<h2>Advanced SAML 1.1 example</h2>
			<?php 
				...
				echo 'redirect_to ' . urlencode($redirect_link);
		        ...
		        foreach ($value as $akey => $avalue) {
		            $redirect_link .= '&'.$akey.'='.$avalue;
		        }
			    } 
				}
				
				echo '<br />redir: <br />' . $redirect_link;
				
				//header( 'Location:'.$redirect_link.$redir_string) ;
